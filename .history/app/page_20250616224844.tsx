...
export default function HotelDirectory() {
  ...
  const uniqueRegions = useMemo(() => {
    ...
    // 나머지 지역을 알파벳 순으로 추가
    const remainingRegions = filteredRegions.filter((region) => !priorityRegions.includes(region)).sort()
  ...
  const uniqueBrands = useMemo(() => {
    ...
    hotels.forEach((hotel) => {
      ...
      normalizedBrandCounts[normalizedBrand] = (normalizedBrandCounts[normalizedBrand] || 0) + 1
  ...
  const filteredHotels = useMemo(() => {
    return hotels.filter((hotel) => {
      ...
      const matchesRegion = filters.regions.length === 0 || filters.regions.includes(hotel.region)

      // 브랜드 필터링 시 정규화된 브랜드명으로 비교
      const normalizedHotelBrand = normalizeBrandName(hotel.brand)
      const matchesBrand = filters.brands.length === 0 || filters.brands.includes(normalizedHotelBrand)
      ...
      <div>
        ...
        <div className="space-y-2">
          {uniqueRegions.map((region) => (
            <div key={region} className="flex items-center justify-between space-x-2">
              ...
              <span className="text-xs text-gray-500">{regionCounts[region || "기타"] || 0}</span>
            </div>
          ))}
        </div>
      </div>
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      <div className="container mx-auto px-4 py-8">
        {/* Header */}
        <div className="mb-8">
          <h1 className="text-4xl font-bold text-gray-900 mb-2">투어비스 럭셔리 셀렉트 호텔 디렉토리</h1>
          ...
          <div className="flex-1">
            ...
            <div className="mb-6">
              <p className="text-gray-600">
                셀렉트 혜택을총 {filteredHotels.length}개의 호텔이 검색되었습니다
